...
- **種目リストは1件ずつ、設定は1項目ずつマージする**（2026-09-22〜）。doc は
  ...
  - **同じ種目が両方にあるときは項目ごと**（`_mergeExercise`）。保存時に変わった項目へ `fieldAt[項目]` を付け、
    項目ごとに時刻が大きいほう（同じならサーバー）。`fieldAt` を持つ種目で載っていない項目は「変えていない」＝0。
    持たない古い種目だけ updatedAt で代用する（持つ種目でも代用すると、別の項目を変えただけで相手の変更に勝ってしまう）
  - マージ（`_mergeStateDocs`）: 墓標の時刻がその種目の
  ...
  - 新しい端末の見本3種目（一度も触っていないもの）は、**初めての同期に限り・サーバーに同じ番号が無いときだけ**混ぜない。
    見本をそのまま使い続けている本物の種目（番号1のベンチプレス）があるので、常に除外すると
    同期のたびに一番下へ回される（2026-09-22 に実際に起きた）
